...
function saveConfig() {
  AppState.config = {
    ...
    aiModel:         document.getElementById('aiModel')?.value || 'gpt-4o-mini',
  };
  Storage.save();
...
function clearAllData() {
  ...
  AppState.config         = { technicianName:'', workDate:'', targetLocations:0,
                               openaiKey:'', companyName:'', aiModel:'gpt-4o-mini' };
  restoreConfig();
...
const PDFParser = {
  ...
  parseSuratJalan(text, filename) {
    ...
    // ── Extended fields ──
    const idMesin = find([
      /ID\s+Mesin\s*[:.]\s*([A-Z0-9\-]+)/i,
      /Mesin\s+ID\s*[:.]\s*([A-Z0-9\-]+)/i,
      /Serial\s+Number\s*[:.]\s*([A-Z0-9\-]+)/i,
    ]);

    const alamatLokasi = find([
      /Alamat\s*[:.]\s*(.+?)(?:\n|No\.|SJ|$)/is,
      /Address\s*[:.]\s*(.+?)(?:\n|SJ|$)/is,
    ]);

    const hariTanggalDiperlukan = find([
      /Hari\s*&\s*Tanggal\s*[:.]\s*(.+?)(?:\n|Teknisi)/is,
      /Diperlukan\s*[:.]\s*(.+?)(?:\n)/i,
    ]);

    const namaTeknisi = find([
      /Teknisi\s*[:.]\s*(.+?)(?:\n)/i,
      /Nama\s*Teknisi\s*[:.]\s*(.+?)(?:\n)/i,
    ]);

    const barangDiperlukan = find([
      /Barang\s*Diperlukan\s*[:.]\s*(.+?)(?:\n{2}|Lain|$)/is,
      /Equipment\s*[:.]\s*(.+?)(?:\n{2}|Lain|$)/is,
    ]);
    ...
    const estimatedArrivalTime = find([
      /Estimated\s+Arrival\s*[:.]\s*(\d{1,2}:\d{2})/i,
      /ETA\s*[:.]\s*(\d{1,2}:\d{2})/i,
    ]);

    const actualArrivalTime = find([
      /Actual\s+Arrival\s*[:.]\s*(\d{1,2}:\d{2})/i,
      /ATA\s*[:.]\s*(\d{1,2}:\d{2})/i,
    ]);
    ...
    return {
      ...
      idMesin,
      alamatLokasi,
      hariTanggalDiperlukan,
      namaTeknisi: namaTeknisi || AppState.config.technicianName,
      barangDiperlukan,
      jmlPB,
      lainLain,
      totalWaktu,
      ...
      tipe:      tipe   || '',
      jmlMesin,
      equipment: barangDiperlukan || equipment || '',
      kendala:   '',
      kendalaText: '',
      filename,
    ...
    <div class="${cardClass}" id="sj-card-${sj.id}">
      <div class="sj-card-header">
        <div>
          ...
          ${sj.idMesin ? `<div class="sj-id-mesin"><i class="bi bi-upc-scan"></i> ID Mesin: ${Utils.escHtml(sj.idMesin)}</div>` : ''}
          <small class="text-muted">${Utils.escHtml(sj.hariTanggalDiperlukan || sj.tanggal || '')}</small>
        </div>
      ...
      <div class="sj-meta">
        ${sj.alamatLokasi ? `<div class="sj-badge"><i class="bi bi-geo-alt"></i> ${Utils.escHtml(sj.alamatLokasi)}</div>` : ''}
        ${sj.namaTeknisi ? `<div class="sj-badge"><i class="bi bi-person"></i> Teknisi: ${Utils.escHtml(sj.namaTeknisi)}</div>` : ''}
        ${sj.barangDiperlukan ? `<div class="sj-badge"><i class="bi bi-tools"></i> ${Utils.escHtml(sj.barangDiperlukan.slice(0,40))}${sj.barangDiperlukan.length>40?'…':''}</div>` : (sj.equipment ? `<div class="sj-badge"><i class="bi bi-tools"></i>${Utils.escHtml(sj.equipment.slice(0,40))}${sj.equipment.length>40?'…':''}</div>` : '')}
        <div class="sj-badge"><i class="bi bi-battery-charging"></i>PB: <strong>${sj.jmlPB}</strong></div>
        ${sj.totalWaktu ? `<div class="sj-badge"><i class="bi bi-clock"></i> ${Utils.escHtml(sj.totalWaktu)}</div>` : ''}
      ...
      <div class="sj-status-bar">
        ...
        <span class="sj-docs-count">ETA: ${Utils.escHtml(sj.estimatedArrivalTime || '-')} | ATA: ${Utils.escHtml(sj.actualArrivalTime || '-')}</span>
        ${sj.notes ? `<span class="kendala-badge"><i class="bi bi-chat"></i> ${Utils.escHtml(sj.notes.substring(0,40))}</span>` : kendalaHtml}
        <span class="ms-auto">
...
function openEditSJ(id) {
  ...
  // Extended fields
  const setIfExists = (elId, val) => { const el = document.getElementById(elId); if (el) el.value = val || ''; };
  setIfExists('edit-sj-id-mesin',     sj.idMesin);
  setIfExists('edit-sj-alamat',       sj.alamatLokasi);
  setIfExists('edit-sj-hari-tanggal', sj.hariTanggalDiperlukan);
  setIfExists('edit-sj-teknisi',      sj.namaTeknisi);
  setIfExists('edit-sj-barang',       sj.barangDiperlukan);
  setIfExists('edit-sj-total-waktu',  sj.totalWaktu);
  setIfExists('edit-sj-address',      sj.address);
  setIfExists('edit-sj-eta',          sj.estimatedArrivalTime);
  setIfExists('edit-sj-ata',          sj.actualArrivalTime);
  setIfExists('edit-sj-lain',         sj.lainLain);
  setIfExists('edit-sj-notes',        sj.notes);
  toggleKendalaText();
...
function saveEditSJ() {
  ...
  SuratJalan.update(id, {
    nomor,
    tanggal:     getVal('edit-sj-tanggal'),
    lokasi,
    mesin:       getVal('edit-sj-mesin'),
    tipe:        getVal('edit-sj-tipe'),
    ...
    equipment:   getVal('edit-sj-equipment'),
    kendala:     getVal('edit-sj-kendala'),
    kendalaText: getVal('edit-sj-kendala-text'),
    // Extended fields
    idMesin:               getVal('edit-sj-id-mesin'),
    alamatLokasi:          getVal('edit-sj-alamat'),
    hariTanggalDiperlukan: getVal('edit-sj-hari-tanggal'),
    namaTeknisi:           getVal('edit-sj-teknisi'),
    barangDiperlukan:      getVal('edit-sj-barang'),
    totalWaktu:            getVal('edit-sj-total-waktu'),
    address:               getVal('edit-sj-address'),
    estimatedArrivalTime:  getVal('edit-sj-eta'),
    actualArrivalTime:     getVal('edit-sj-ata'),
    lainLain:              getVal('edit-sj-lain'),
    notes:                 getVal('edit-sj-notes'),
  });
...
============================================================ */
const AI = {
  /** Build the context prompt from current state */
  buildContext() {
    const kpi = KPI.compute();
    ...
    const sjDetails = sjs.map((s, i) => {
      const k = AppState.kendalaMap[s.id] || { type: s.kendala||'', text: s.kendalaText||'' };
      const ba = AppState.beforeAfterMap[s.id] || { before: [], after: [] };
      return `${i+1}. ${s.nomor} — ${s.lokasi} | Mesin: ${s.mesin||'-'} (${s.jmlMesin}) | PB: ${s.jmlPB}` +
        (ba.before.length||ba.after.length ? ` | Before/After: ${ba.before.length}/${ba.after.length} foto` : '') +
        (k.type ? ` | Kendala: ${k.type}${k.text?' - '+k.text:''}` : '');
    }).join('\n');

    const kendalaList = sjs
      .map(s => AppState.kendalaMap[s.id] || { type: s.kendala||'' })
      .filter(k => k.type)
      .map((k, i) => `- ${sjs[i]?.lokasi||'Lokasi'}: ${k.type}${k.text?' ('+k.text+')':''}`)
      .join('\n') || 'Tidak ada kendala';

    return `
Laporan Harian Deployment:
Teknisi: ${cfg.technicianName || 'Teknisi'}
Tanggal: ${cfg.workDate || new Date().toLocaleDateString('id-ID')}
Perusahaan: ${cfg.companyName || '-'}

KPI:
- Total Surat Jalan: ${kpi.totalSJ}
- Total Lokasi: ${kpi.totalLokasi}
- Lokasi Selesai: ${kpi.lokasiSelesai}
- Lokasi Belum Lengkap: ${kpi.lokasiBelum}
- Total Mesin Terpasang: ${kpi.totalMesin}
- Total Powerbank: ${kpi.totalPB}
- Total Foto Dokumentasi: ${kpi.totalFoto}
- Jam Mulai: ${kpi.startTime}
- Jam Selesai: ${kpi.finishTime}
- Durasi Kerja: ${kpi.duration}
- Jarak Tempuh: ${kpi.distance}
- Productivity Score: ${kpi.productivityScore !== null ? kpi.productivityScore.toFixed(1)+'%' : '—'} (${kpi.productivityCategory})

Asset Tracking:
- PB Dibawa: ${asset.pbDibawa||0}
- PB Terpasang: ${asset.pbTerpasang||0}
- PB Rusak: ${asset.pbRusak||0}
- PB Tersisa: ${asset.pbSisa||0}

Kendala:
${kendalaList}

Detail Surat Jalan:
${sjDetails || 'Tidak ada data SJ'}
    `.trim();
  },

  /** Call OpenAI API */
  async callOpenAI(systemPrompt, userPrompt) {
    const apiKey = AppState.config.openaiKey;
    if (!apiKey) throw new Error('OpenAI API Key belum diisi di konfigurasi.');
    ...
    const response = await fetch('https://api.openai.com/v1/chat/completions', {
      ...
      body: JSON.stringify({
        model:      AppState.config.aiModel || 'gpt-4o-mini',
        max_tokens: 1000,
        messages: [
  ...
  prompts: {
    aktivitas: {
      system: 'Kamu adalah asisten laporan harian deployment teknisi. Tulis dalam Bahasa Indonesia, profesional, padat, dan terstruktur. Fokus pada aktivitas lapangan.',
      user:   (ctx) => `Berdasarkan data berikut, tulis ringkasan AKTIVITAS harian deployment (3-5 kalimat, mencakup jumlah lokasi dikunjungi, mesin dipasang, dan aktivitas utama):\n\n${ctx}`,
    },
    hasil: {
      system: 'Kamu adalah asisten laporan harian deployment teknisi. Tulis dalam Bahasa Indonesia, profesional, padat. Fokus pada pencapaian dan hasil.',
      user:   (ctx) => `Berdasarkan data berikut, tulis ringkasan HASIL deployment harian (3-5 kalimat, mencakup pencapaian, produktivitas, dan kondisi perangkat):\n\n${ctx}`,
    },
    kesimpulan: {
      system: 'Kamu adalah asisten laporan harian deployment teknisi senior. Tulis dalam Bahasa Indonesia, formal, ringkas, dan profesional.',
      user:   (ctx) => `Berdasarkan data berikut, tulis KESIMPULAN profesional laporan harian deployment (2-4 kalimat, mencakup evaluasi keseluruhan dan catatan penting):\n\n${ctx}`,
    },
    insight: {
      system: 'Kamu adalah analis operasional deployment teknisi. Tulis dalam Bahasa Indonesia, actionable, dan berbasis data.',
      user:   (ctx) => `Berdasarkan data berikut, tulis AI INSIGHT DAN REKOMENDASI operasional (format bullet points: 3-5 insight + 3-5 rekomendasi konkret):\n\n${ctx}`,
    },
  },
...
async function generateAISection(section) {
  const prompts = AI.prompts[section];
  if (!prompts) return;
  ...
  try {
    ...
    const result  = await AI.callOpenAI(prompts.system, prompts.user(context));
    ...
    Utils.toast(`Error: ${err.message}`, 'danger', 5000);
    console.error(err);
  } finally {
...
async function generateAllAI() {
  ...
  for (const section of ['aktivitas', 'hasil', 'kesimpulan', 'insight']) {
      ...
      <div class="review-section mb-2">
        ...
        <div class="review-section-body">
          <table class="review-table">
            ...
            ${row('ID Mesin', sj.idMesin)}
            ${row('Mesin', `${sj.mesin||'—'} (${sj.tipe||'—'})`)}
            ${row('Jumlah Mesin', sj.jmlMesin)}
            ${row('Jumlah Powerbank', sj.jmlPB)}
            ${row('Alamat Lokasi', sj.alamatLokasi)}
            ${row('Hari & Tanggal Diperlukan', sj.hariTanggalDiperlukan)}
            ${row('Teknisi', sj.namaTeknisi)}
            ${row('Barang Diperlukan', sj.barangDiperlukan)}
            ${row('Equipment', sj.equipment)}
            ${row('Lain-lain', sj.lainLain)}
            ${row('Total Waktu', sj.totalWaktu)}
            ${row('Address', sj.address)}
            ${row('Estimated Arrival', sj.estimatedArrivalTime)}
            ${row('Actual Arrival', sj.actualArrivalTime)}
            ${row('Notes', sj.notes)}
        ...
        <div class="sj-block">
          ...
          <div class="sj-block-body">
            <table>
              ...
              ${sj.idMesin ? `<tr><td class="td-label">ID Mesin</td><td>${Utils.escHtml(sj.idMesin)}</td></tr>` : ''}
              ...
              ${sj.alamatLokasi ? `<tr><td class="td-label">Alamat Lokasi</td><td>${Utils.escHtml(sj.alamatLokasi)}</td></tr>` : ''}
              ${sj.hariTanggalDiperlukan ? `<tr><td class="td-label">Hari &amp; Tanggal</td><td>${Utils.escHtml(sj.hariTanggalDiperlukan)}</td></tr>` : ''}
              ${sj.namaTeknisi ? `<tr><td class="td-label">Teknisi</td><td>${Utils.escHtml(sj.namaTeknisi)}</td></tr>` : ''}
              ${sj.barangDiperlukan ? `<tr><td class="td-label">Barang Diperlukan</td><td>${Utils.escHtml(sj.barangDiperlukan)}</td></tr>` : ''}
              <tr><td class="td-label">Equipment</td><td>${Utils.escHtml(sj.equipment||'—')}</td></tr>
              ${sj.lainLain ? `<tr><td class="td-label">Lain-lain</td><td>${Utils.escHtml(sj.lainLain)}</td></tr>` : ''}
              ${sj.totalWaktu ? `<tr><td class="td-label">Total Waktu</td><td>${Utils.escHtml(sj.totalWaktu)}</td></tr>` : ''}
              ${sj.address ? `<tr><td class="td-label">Address</td><td>${Utils.escHtml(sj.address)}</td></tr>` : ''}
              ${sj.estimatedArrivalTime ? `<tr><td class="td-label">Estimated Arrival</td><td>${Utils.escHtml(sj.estimatedArrivalTime)}</td></tr>` : ''}
              ${sj.actualArrivalTime ? `<tr><td class="td-label">Actual Arrival</td><td>${Utils.escHtml(sj.actualArrivalTime)}</td></tr>` : ''}
              ${sj.notes ? `<tr><td class="td-label">Notes</td><td>${Utils.escHtml(sj.notes)}</td></tr>` : ''}
